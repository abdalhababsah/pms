                ...
                <div class="col-xl-4 col-lg-5">
                    <div class="card h-100">
                        <div class="card-body text-center">
                            <img src="{{ asset('assets/images/UserAvatar.png') }}" alt="Avatar"
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var ratingSeries = Object.keys(ratingData).map(key => ({
            ...
            projectChart.setOption({
                xAxis: {
                    type: 'category',
                    data: {!! json_encode($projectFeedback->pluck('project.name')) !!}
                },
                yAxis: {
                    type: 'value'
                },
                series: [{
                    type: 'bar',
                    data: {!! json_encode($projectFeedback->pluck('avg_rating')) !!}
                }]
            });

            function loadTimeline(filterDate) {
        // Call the backend route with the filter_date query parameter.
        fetch("{{ route('employee.worklog.timeline', $employee->id) }}?filter_date=" + encodeURIComponent(filterDate))
            .then(response => response.json())
            .then(data => {
                let container = document.getElementById('timelineContainer');
                let html = '';

                // Check if a work day record was found for this date.
                if (data.workDay) {
                    html += `<div class="mb-3">
                        <strong>Date:</strong> ${new Date(data.workDay.date).toLocaleDateString()}<br>
                        <strong>Location:</strong> ${data.workDay.location}<br>
                        <strong>Shift:</strong> ${data.workDay.shift.name}
                    </div>`;

                    // Check if there are work logs for that day.
                    if (data.workLogs && data.workLogs.length > 0) {
                        // Group logs by project if needed or simply list each log.
                        html += '<ul class="list-group">';
                        data.workLogs.forEach(function(log) {
                            html += `<li class="list-group-item">
                                <div><strong>Project:</strong> ${log.project ? log.project.name : 'N/A'}</div>
                                <div><strong>Task:</strong> ${log.task_description}</div>
                                <div class="small text-muted">
                                    <strong>Task Count:</strong> ${log.task_count} | 
